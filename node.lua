...
local function wrap(str, width, font, size)
    local text = ""
    local splitted = {}
    for word in str:gmatch("%S+") do
        if font:width(text, size) < width then
            local testtext = text .. " " .. word
            if font:width(testtext, size) > width then
                splitted[#splitted + 1] = text
                text = ""
            end
            text = text .. " " .. word
        end
...
local function Clock()
    local human_time = ""
    local unix_diff = 0
    local base_seconds = 0

    util.data_mapper{
        ["clock/seconds"] = function(seconds)
            base_seconds = tonumber(seconds) - sys.now()
        end;
    ...
    local function time()
        return math.floor(base_seconds + sys.now())
    end
...
local function Markup(config)
        ...
        for line in string.gmatch(paragraph, "[^\n]+") do
            ...
            if line:find "|" then
                ...
                elseif line:sub(1,4) == "!###" then
                    ...
                    font = font_regl
                    size = 50
                    tmp_x = (width-font:width(line, size))/2
                    centered = true
                ...
                local chunks = wrap(line, width-80, font, size)
                for idx = 1, #chunks do
    ...
    local base_x = 40
    local base_y = 30
    if config.centered and y < height then
        base_y = (height-y)/2
    end
    ...
    return function(starts, ends)
        for now, x1, y1, x2, y2 in from_to(starts, ends) do
            local x = x1 + base_x
            local y = y1 + base_y
            -- overlays[1]:draw(x1, y1, x2, y2, 0.1)
            for idx = 1, #writes do
                local w = writes[idx]
                if w.centered then
                    w.font:write(x1+w.x, y+w.y, w.text, w.size, r,g,b,1)
                else
...
local function playlist()
    ...
    local function tile_real_fullscreen(s, e, now)
        return 0, 0, WIDTH, HEIGHT
    end

    local function tile_fullscreen(s, e, now)
        return 0, 150, WIDTH, HEIGHT
    end

    local function tile_fullscreen_center(s, e, now)
        return 220, 215, 1700, 915
    end
    ...
    local function tile_bottom_scroller(s, e, now)
        return 500, HEIGHT-50, WIDTH, HEIGHT
    end

    local function tile_bottom_clock(s, e, now)
        return 0, HEIGHT-50, 500, HEIGHT
    end

    local function tile_top_clock(s, e, now)
        return 1375, 50, 1875, 100
    end

    local function tile_right(s, e, now)
        return WIDTH/2, 150, WIDTH, HEIGHT
    end

    local function tile_left(s, e, now)
        return 0, 150, WIDTH/2, HEIGHT
    end
    ...
    local function add_clock(page, duration)
      add{
          offset = offset,
          duration = duration,
          fn = TimeTile{
    ...
    local function page_fullscreen(page)
        ...
        add{
            ...
            coord = tile_real_fullscreen,
        }
        add_info_bar(page, duration)
        offset = offset + duration
    end

    local function page_text_center(page)
        local duration = get_duration(page)
        add{
            offset = offset,
            duration = duration,
        ...
        add{
            ...
            fn = Markup{
                ...
                width = 1480,
                height = 700,
                color = page.config.foreground or "#000000",
                centered = false,
            },
            coord = tile_fullscreen_center,
        }
        add_info_bar(page, duration)
        add_clock(page, duration)
        offset = offset + duration
    end

    local function page_text_delivery(page)
        local duration = get_duration(page)
        local name = page.config.name or ""
        local model = page.config.model or ""
        local delivery_text = "!#Herzlich Willkommen\n\n\n\n!##" .. name
        if model ~= "" then
    ...
    local function page_text_left(page)
        ...
        add{
            ...
            fn = Flat{
                fade_time = 0,
                color = page.config.background or "#dadada",
            },
            coord = tile_left,
        }
        add{
    ...
    local layouts = {
        ...
        ["text-center"] = page_text_center;
        ["text-delivery"] = page_text_delivery;
    }

    for idx = 1, #config.pages do
        local page = config.pages[idx]
        local earlierTime = tonumber(page.schedule.earlierTime)
        local laterTime = tonumber(page.schedule.laterTime)
        print("Playlist time: " .. time)
        if earlierTime < time and time < laterTime then
            layouts[page.layout](page)
            print("page " .. idx .. " scheduled between " .. earlierTime .. " and " .. laterTime)
        else
            print("page " .. idx .. " not scheduled just between " .. earlierTime .. " and " .. laterTime)
        end
    end
    return playlist
end
